...
public:
    ...
    std::pair<std::vector<float>, std::vector<float>> process_spectrogram(
        const std::vector<std::vector<float>>& spectrogram, std::function<bool()> shouldCancel = nullptr);
...
private:
    ...
    std::vector<std::vector<std::vector<float>>> split_piece(const std::vector<std::vector<float>>& spect,
                                                             int chunk_size, int border_size);
    ...
    std::pair<std::vector<float>, std::vector<float>> aggregate_prediction(
        ...
        const std::vector<int>& starts, int full_size, int chunk_size, int border_size);
