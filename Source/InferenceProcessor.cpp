...
InferenceProcessor::InferenceProcessor(Ort::Session& session, Ort::Env& env) : session_(session), env_(env) {}
...
std::pair<std::vector<float>, std::vector<float>> InferenceProcessor::run_onnx_inference(
    ...
    for (size_t i = 0; i < chunk_spect.size(); ++i) {
        memcpy(input_tensor_values.data() + i * chunk_spect[0].size(), chunk_spect[i].data(),
               chunk_spect[0].size() * sizeof(float));
    }
...
std::vector<std::vector<float>> zeropad(const std::vector<std::vector<float>>& spect, int left, int right) {
    if (left == 0 && right == 0) {
        return spect;
    } else {
        int original_rows = spect.size();
        int original_cols = spect[0].size();
        std::vector<std::vector<float>> padded_spect(original_rows + left + right,
                                                     std::vector<float>(original_cols, 0.0f));
...
std::vector<std::vector<std::vector<float>>> InferenceProcessor::split_piece(
    const std::vector<std::vector<float>>& spect, int chunk_size, int border_size) {
...
std::pair<std::vector<float>, std::vector<float>> InferenceProcessor::aggregate_prediction(
    const std::vector<std::pair<std::vector<float>, std::vector<float>>>& pred_chunks, const std::vector<int>& starts,
    int full_size, int chunk_size, int border_size) {
...
std::pair<std::vector<float>, std::vector<float>> InferenceProcessor::process_spectrogram(
    const std::vector<std::vector<float>>& spectrogram, std::function<bool()> shouldCancel) {
    ...
    int total_frames = spectrogram.size();
    if (total_frames == 0) return {{}, {}};

    std::vector<int> starts;  // This declaration was in the original code, keeping it.
    ...
    for (size_t i = 0; i < chunks.size(); ++i) {
        if (shouldCancel && shouldCancel()) return {{}, {}};
        pred_chunks.push_back(run_onnx_inference(chunks[i]));
    }
