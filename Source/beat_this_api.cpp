...
#include <locale>
    ...
    Impl(const void* model_data, size_t model_size) : env(ORT_LOGGING_LEVEL_WARNING, "beat_this_cpp_api") {
        ...
        try {
            session = std::make_unique<Ort::Session>(env, model_data, model_size, session_options);
        } catch (const Ort::Exception& e) {
            std::cerr << "ONNX Runtime error during session creation from memory: " << e.what() << std::endl;
            std::cerr << "Error code: " << e.GetOrtErrorCode() << std::endl;
            throw std::runtime_error("ONNX Runtime error: " + std::string(e.what()));
        }
    }
...
std::vector<int> calculate_beat_counts(const std::vector<float>& beats, const std::vector<float>& downbeats) {
    ...
    if (downbeats.size() >= 2) {
        ...
        if (pickup_beats < beats_in_first_measure) {
            ...
            std::cerr << "WARNING: There are more beats in the pickup measure than "
                         "in the first measure. "
                      << "The beat count will start from 2 without trying to "
                         "estimate the length of the pickup measure."
                      << std::endl;
            start_counter = 1;
        }
    } else {
        std::cerr << "WARNING: There are less than two downbeats in the "
                     "predictions. Something may be wrong. "
                  << "The beat count will start from 2 without trying to estimate "
...
bool resample_audio(const std::vector<float>& in_buffer, int in_rate, std::vector<float>& out_buffer, int out_rate) {
    ma_resampler resampler;
    ma_resampler_config resampler_config = ma_resampler_config_init(ma_format_f32, 1, (ma_uint32)in_rate,
...
BeatResult BeatThis::process_audio(const std::vector<float>& audio_data, int samplerate, int channels,
    ...
    try {
        ...
        auto beat_downbeat_times = postprocessor.process(beat_downbeat_logits.first, beat_downbeat_logits.second);

        // Calculate beat counts
        std::vector<int> beat_counts = calculate_beat_counts(beat_downbeat_times.first, beat_downbeat_times.second);

        return BeatResult{std::move(beat_downbeat_times.first), std::move(beat_downbeat_times.second),
